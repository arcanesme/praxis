...
const CLIENTS_DIR = path.join(PROMPTS_DIR, 'clients');
...
/**
 * Discover all deals across all clients.
 * Returns array of { client, deal, dealDir, clientDir }.
 */
function discoverAllDeals() {
  ...
  const clients = fs.readdirSync(CLIENTS_DIR)
    .filter((d) => fs.statSync(path.join(CLIENTS_DIR, d)).isDirectory());
 ...
 * Accepts: "deal-name", "client/deal-name", or a full path.
 * Searches clients hierarchy first, then falls back to legacy prompts/projects/.
 */
function resolveProject(identifier) {
  // Handle explicit "client/deal" format
  if (identifier.includes('/')) {
    const [clientName, dealName] = identifier.split('/', 2);
    const dealDir = path.join(CLIENTS_DIR, clientName, 'deals', dealName);
    const clientDir = path.join(CLIENTS_DIR, clientName);
    if (fs.existsSync(dealDir)) {
      return { client: clientName, deal: dealName, dealDir, clientDir, legacy: false };
    }
  }

  // Search all clients for a matching deal name
